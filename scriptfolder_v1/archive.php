...
?>
    ...
    <div style="text-align:<?php echo $OptionsVis["link_align"]; ?>"><a href="<?php echo $thisPage; ?>?cat_id=<?php echo $_REQUEST["cat_id"]; ?>&amp;p=<?php echo $_REQUEST["p"]; ?>" style='font-weight:<?php echo $OptionsVis["link_font_weight"]; ?>;color:<?php echo $OptionsVis["link_color"]; ?>;font-size:<?php echo $OptionsVis["link_font_size"]; ?>;text-decoration:underline'><?php echo $OptionsLang["Back_to_home"]; ?></a></div>    
    <div style="clear:both; height:<?php echo $OptionsVis["dist_link_title"];?>;"></div>    
    <?php } ?>

	<?php 
	$sql = "SELECT * FROM ".$TABLE["Archives"]." WHERE status='Published' AND id='".mysql_real_escape_string($_REQUEST["id"])."'";
	$sql_result = mysql_query ($sql, $conn ) or die ('MySQL query error: '.$sql.'. Error: '.mysql_error());
    ...
    <div style="clear:both"></div>
        ...
        $sql = "SELECT * FROM ".$TABLE["Comments"]." WHERE archive_id='".$News["id"]."' AND status='Approved' ORDER BY id ".$commentOrder;
        $sql_result = mysql_query ($sql, $conn ) or die ('MySQL query error: '.$sql.'. Error: '.mysql_error());
...
?>
	<div>  	
		...
		$sql = "SELECT count(*) as total FROM ".$TABLE["Archives"]." WHERE status='Published' ".$search;
		$sql_result = mysql_query ($sql, $conn ) or die ('MySQL query error: '.$sql.'. Error: '.mysql_error());
		...
		$sql = "SELECT * FROM ".$TABLE["Archives"]."  
         ...
         <tr>
           ...
           <td valign="top" align="left" width="<?php echo $OptionsVis["summ_img_width"];?>">
            ...
            <div style="padding-right:10px; padding-top:3px;">
            	<a href="<?php echo $thisPage; ?>?id=<?php echo $News['id']; ?>&amp;cat_id=<?php echo $_REQUEST["cat_id"]; ?>&amp;p=<?php echo $_REQUEST["p"]; ?>#ontitle">
        ...
        <?php } // end if shownews == TitleAndSummary - for image ?>    
			...
			<div style="color:<?php echo $OptionsVis["summ_color"];?>; font-family:<?php echo $OptionsVis["summ_font"];?>; font-size:<?php echo $OptionsVis["summ_size"];?>;font-style: <?php echo $OptionsVis["summ_font_style"];?>;text-align:<?php echo $OptionsVis["summ_text_align"];?>;line-height:<?php echo $OptionsVis["summ_line_height"];?>;">
             ...
             <a style="color:<?php echo $OptionsVis["summ_title_color"];?>; text-decoration: underline" href="<?php echo $thisPage; ?>?id=<?php echo $News['id']; ?>&amp;cat_id=<?php echo $_REQUEST["cat_id"]; ?>&amp;p=<?php echo $_REQUEST["p"]; ?>#ontitle"><?php echo $OptionsLang['Read_more']; ?></a>
        	</div>
        ...
        <div class="pagination">
            ...
            if ($page > 1) {
            ?>	 
            <a class="next_prev" href="<?php echo $thisPage."?p=".$prev;?>&search=<?php echo urlencode($_REQUEST["search"]);?>"><?php echo $OptionsLang["Previous"];?></a>
            <?php } else { ?>
            ...
            if ($lastpage < 7 + ($adjacents * 2)) {	//not enough pages to bother breaking it up
                for ($counter = 1; $counter <= $lastpage; $counter++) {
                    ...
                    <?php } else { ?>
                    <a href="<?php echo $thisPage."?p=".$counter; ?>&search=<?php echo urlencode($_REQUEST["search"]);?>"><?php echo $counter; ?></a>
                    <?php } 				
                }
            }
            elseif($lastpage > 5 + ($adjacents * 2)) {	//enough pages to hide some		
                ...
                if($page < 1 + ($adjacents * 2)) {
                    ...
                    ...<a href="<?php echo $thisPage."?p=".$lpm1; ?>&search=<?php echo urlencode($_REQUEST["search"]);?>"><?php echo $lpm1; ?></a>
                    <a href="<?php echo $thisPage."?p=".$lastpage; ?>&search=<?php echo urlencode($_REQUEST["search"]);?>"><?php echo $lastpage; ?></a>
                    
                <?php   
                } elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2)) { //in middle; hide some front and some back ?>
                    <a href="<?php echo $thisPage; ?>?p=1&search=<?php echo urlencode($_REQUEST["search"]); ?>">1</a>
                    <a href="<?php echo $thisPage; ?>?p=2&search=<?php echo urlencode($_REQUEST["search"]); ?>">2</a>...
                    ...
                    for ($counter = $page - $adjacents; $counter <= $page + $adjacents; $counter++) {
                        if ($counter == $page) { ?>
                            <span class="current"><?php echo $counter; ?></span>
                    <?php } else { ?>
                            <a href="<?php echo $thisPage."?p=".$counter; ?>"><?php echo $counter; ?></a>	
                    <?php } 
                                            
                    } ?>
                    ...<a href="<?php echo $thisPage."?p=".$lpm1; ?>"><?php echo $lpm1; ?></a>
                    <a href="<?php echo $thisPage."?p=".$lastpage; ?>"><?php echo $lastpage; ?></a>
                <?php     		
            ...
            //next button
            if ($page < $counter - 1) { ?>
                <a class="next_prev" href="<?php echo $thisPage."?p=".$next; ?>&search=<?php echo urlencode($_REQUEST["search"]); ?>"><?php echo $OptionsLang["Next"]; ?></a>
            <?php } else { ?>
